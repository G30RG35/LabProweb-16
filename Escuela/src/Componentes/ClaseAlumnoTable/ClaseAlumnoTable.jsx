...
const ClaseAlumnoTable = ({ users, infoExcel = [], setAlumnos }) => {
    ...
    const handleDeleteUser = async(userId) => {
        ...
        try {
            const { data } = await axios.delete(`${import.meta.env.VITE_API_URL}/api/clasesAlumnos/${user[0].grupoID}/${user[0].materiaID}/${user[0].maestroID}/${user[0].usuarioID}`, config)
            
            const newArray = users.filter(user => user.usuarioID !== +userId)
            setAlumnos(newArray)
            setAlerta({
                error : false, 
                msg : data.msg
